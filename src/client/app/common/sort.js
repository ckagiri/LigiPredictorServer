...
(function (app) {
    ...
    (function (core) {
        ...
        var SortService = (function () {
            ...
            SortService.prototype.sort_by = function (field, reverse) {
                return function (a, b) {
                    var A = a[field], B = b[field];
                    return naturalSort(A, B, { insensitive: true, desc: !!reverse });
                };
            };
            return SortService;
        }());
        core.SortService = SortService;
        angular
            .module('app.core')
...
function naturalSort(a, b, options) {
    var re = /(^-?[0-9]+(\.?[0-9]*)[df]?e?[0-9]?$|^0x[0-9a-f]+$|[0-9]+)/gi, sre = /(^[ ]*|[ ]*$)/g, dre = /(^([\w ]+,?[\w ]+)?[\w ]+,?[\w ]+\d+:\d+(:\d+)?[\w ]?|^\d{1,4}[\/\-]\d{1,4}[\/\-]\d{1,4}|^\w+, \w+ \d+, \d{4})/, hre = /^0x[0-9a-f]+$/i, ore = /^0/, options = options || {}, i = function (s) { return options.insensitive && ('' + s).toLowerCase() || '' + s; }, 
